...
static void test_atVector_first() {
  printf("\t\t [ first ] \t -> \t ");
...
static void test_atVector_middle() {
  printf("\t\t [ middle ] \t -> \t ");

  vector v = createVector(NORMAL);
  v.size = v.capacity;
  const size_t i = NORMAL / 2;
  assert(atVector(&v, i) == v.data + i);
  destroyVector(&v);
...
static void test_atVector_last() {
  printf("\t\t [ last ] \t -> \t ");
...
static void test_atVector() {
  printf("\t [ atVector() ]\n");

  test_atVector_first();
  test_atVector_middle();
  test_atVector_last();
}
